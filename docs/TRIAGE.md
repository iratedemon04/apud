...
- [x] **B2 — `MainForm.cs` god-object (DEFERRED 2026-08-02 structural). DECIDED 2026-08-06: DO IT before Module 12 — see `docs/MAINFORM-REFACTOR-PLAN.md`.** Not a triage line item any more; it's a committed sibling task (own plan doc). The user chose to pay the ~1 day because Apud is open source and `MainForm.cs` (2,488 lines, ~no unit tests) is the file new contributors hit first. Mechanical extraction (Sync → Search → Catalogue → maybe Menu), one at a time, build-green + drive + commit each. No decision left here — just execute the plan.
...
3. **MainForm refactor** — execute `docs/MAINFORM-REFACTOR-PLAN.md` (decided pre-M12; ~1 day, open-source navigability).
4. Then **`apud module 12 GO`** — read STATE.md + `docs/PLAN.md` §11 (M8 row = installer) + §2 (Inno Setup); present the step list; build. Decide the packaging fork there. Then a testing round → tag `v1.0.0-rc.1` → fresh-Windows-install acceptance drive → re-tag the same commit `v1.0.0` on green. Bump `<Version>` 0.1.0 → 1.0.0.
